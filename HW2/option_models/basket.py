...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        '''
        prices = forward[:,None]*np.exp(-0.5*np.square(vol[:,None])*texp+np.sqrt(texp)*chol_m@znorm_m)
    else:
...
def basket_price_norm_analytic(
    ...
    div_fac = np.exp(-texp*divr)
    disc_fac = np.exp(-texp*intr)
    forward_basket = (spot/disc_fac*div_fac)@weights

    cov_m = vol*cor_m*vol[:,None]
    vol_basket = np.sqrt(weights@cov_m@weights)
    
    price_ana = normal_formula(strike,forward_basket,vol_basket,texp,0,0,cp_sign)
